...
boot(function (err, botconnect) {
  if (err) {
    throw err
  }
  program.version(botconnect.version)
  ...
  fs.readdir(command_directory, function(err, files){
    ...
    commands.forEach((file)=>{
      require(path.resolve(__dirname, file.replace('.js','')))(program, botconnect.conf)
    })
